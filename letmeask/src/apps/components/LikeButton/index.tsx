...
import { LikeQuestions } from "../../services/apiQuestions";
...
export default function LikeButton({ question }: QuestionProps) {
    ...
    return (
        <>
            <button className={`like-button ${(isLike === 'true') ? 'liked' : ''} `}
                type='button'
                aria-label='Marcar como Gostei'
                onClick={() => { handlerLikeQuestion(question.id) }}
            >
                <span>{contLike > 0 ? contLike : ""}</span>
                <span>{false}</span>
